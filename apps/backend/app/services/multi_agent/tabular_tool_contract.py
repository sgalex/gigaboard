...
## Pull-контекст пайплайна (если в промпте урезаны источники или история)
Когда нужен **полный текст страницы** после discovery/research, а в `sources` в промпте только snippet/meta — вызови **`expandResearchSourceContent`**: `arguments`: `agent_result_index` (int — индекс записи в `agent_results` того шага), `source_index` (int, 0-based в массиве `sources`) или `url` (подстрока для поиска источника), опционально `max_chars`.
Когда не хватает **деталей прошлого шага** (narrative/findings/таблицы в усечённом виде) — **`expandAgentResult`**: `arguments`: `agent_result_index`, опционально `max_total_chars`.
Для узла **context graph** по UUID из блока PIPELINE CONTEXT GRAPH — **`expandContextGraphNode`**: `arguments`: `node_id`, опционально `body_level` = `full` | `compact` | `minimal`.
Индексы `agent_result_index` согласуй с порядком в истории пайплайна / полем `agent_result_index` в узле графа. Повторные идентичные запросы кэшируются.
"""
