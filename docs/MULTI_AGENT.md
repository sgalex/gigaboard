...
> **2026** — система реализована: 9 core-агентов + satellite-контроллеры (включая `ResearchController` для источника AI Research).
...
- выполняется **повторный вызов** LLM с явным требованием вернуть **только валидный JSON** (механизм `_call_gigachat_with_json_retry` в `BaseAgent`);
