# 🔄 DataNode Prompt Processing via Multi-Agent System
...
При создании **текстового DataNode** пользователь вводит естественный промпт, который сохраняется в поле `text_content`. После создания узла, **Multi-Agent система** автоматически анализирует промпт и выполняет соответствующие действия.
...
```typescript
// Frontend: CreateDataNodeDialog.tsx
case 'text':
    dataNodeType = 'text'
...
1. Пользователь создаёт DataNode с промптом
...
class DataNodePromptProcessor:
    """
    Обрабатывает промпты текстовых DataNode через Multi-Agent систему.
    """
    ...
    async def process_prompt(
        ...
        4. Результат сохраняется в DataNode.data
        ...
        if task_type == 'generate':
            ...
            result = await self.transformation.create_transformation(
                prompt, 
                board_context['data_nodes']
            )
        
        # Шаг 3: Сохранение результата
        await self.save_result_to_datanode(data_node_id, result)
        
        # Шаг 4: Broadcast через Socket.IO
        await self.broadcast_datanode_update(data_node_id)
...
```python
# apps/backend/app/routes/data_nodes.py

@router.post(
    "/boards/{board_id}/data-nodes/{node_id}/process",
    response_model=DataNodeResponse,
)
async def process_datanode_prompt(
    board_id: UUID,
    node_id: UUID,
    ...
    """
    Запускает Multi-Agent обработку промпта DataNode.
    Работает асинхронно в фоновом режиме.
    """
    
    # Проверяем что DataNode существует
    data_node = await db.get(DataNode, node_id)
    if not data_node or not data_node.needs_multi_agent_processing:
...
```typescript
// apps/web/src/components/canvas/DataNodeCard.tsx

export function DataNodeCard({ node }: { node: DataNode }) {
    ...
    if (isProcessing) {
        return <DataNodeProcessingIndicator prompt={node.text_content} />
    }
    
    return <DataNodeCardContent node={node} />
}
```
...
```
1. User создаёт DataNode → тип "Текст / Промпт"
   ↓
...
5. Socket.IO получает событие `datanode:processed`
   ↓
...
- Новый DataNode с объединёнными данными
...
class PromptProcessingLog(Base):
    ...
    data_node_id = Column(UUID, ForeignKey("data_nodes.id"))
    prompt = Column(Text)
