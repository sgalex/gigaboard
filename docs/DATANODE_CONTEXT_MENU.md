...
При активации (выделении) **DataNode** на канвасе рядом с узлом появляется контекстное меню с перечнем доступных операций. Меню группирует операции по категориям и предоставляет быстрый доступ к ключевым функциям работы с данными.
...
- **Множественные виджеты**:
  ...
  - Все виджеты размещаются рядом с DataNode

**Результат**: 
- Создаётся WidgetNode с `parent_data_node_id`
- Создаётся VISUALIZATION edge: DataNode → WidgetNode
- WidgetNode размещается на канвасе справа от DataNode
...
- Настроить авто-обновление:
  - При изменении DataNode все виджеты обновляются автоматически
...
**Одиночная трансформация** (DataNode → новый DataNode):
...
- Создаётся новый DataNode с результатом
- Создаётся TRANSFORMATION edge: source DataNode → target DataNode

**Множественная трансформация** (несколько DataNode → один):
- Выбрать другие DataNode как дополнительные источники
...
#### **Автоматизация**
- **Авто-replay**: 
  - При обновлении source DataNode автоматически пересчитывать все downstream трансформации
...
- Markdown поддержка
- Создаётся CommentNode с `target_node_id` = этот DataNode
- Создаётся COMMENT edge: CommentNode → DataNode
- CommentNode размещается рядом с DataNode
...
- Процесс:
  1. Выбрать целевой DataNode на канвасе (detail level)
  2. Настроить фильтрацию при переходе
  3. Создаётся DRILL_DOWN edge: summary DataNode → detail DataNode
...
- При клике автоматически создаётся filtered DataNode или открывается существующий detail node
...
- Примеры:
  ...
  - Связь к другому DataNode с методологией
  - Связь к WidgetNode с reference documentation
- Создаётся REFERENCE edge: этот DataNode → целевой узел
...
#### **Просмотр связей**
- Список всех edges связанных с этим DataNode:
...
- **SQL DataNode**: 
  - Открыть SQL editor
  - Изменить query
  - Re-execute и обновить DataNode
...
- Refresh DataNode с новыми параметрами
...
- Trigger-based updates:
  ...
  - Event-based (изменение другого DataNode → refresh)
...
#### **Поделиться**
- **Public link**: создать публичную ссылку на DataNode
  - View-only режим
...
#### **Открыть в редакторе**
- **SQL Editor**: для SQL DataNodes
  - Syntax highlighting
  - Query execution
  - Results preview
  
- **API Explorer**: для API DataNodes
...
1. User выделяет DataNode "Sales CSV"
...
1. User выделяет DataNode "Raw Orders"
...
6. Создаётся новый DataNode "Filtered Orders" + TRANSFORMATION edge
7. User настраивает авто-replay: при обновлении Raw Orders автоматически пересчитывать Filtered

### Сценарий 3: Drill-Down

1. User создал WidgetNode "Sales by Region" (pie chart) из DataNode "Sales Summary"
2. Выделяет DataNode "Sales Summary"
3. Клик "🔗 Настроить Drill-Down"
4. Выбирает target DataNode "Sales Details"
...
Контекстное меню DataNode обеспечивает:
...
Меню является центральным интерфейсом для работы с данными на канвасе и реализует все ключевые возможности системы DataNode → Transformation → WidgetNode.
