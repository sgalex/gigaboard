...
```json
{
  ...
  "context": {
    ...
    "previous_results": {
      "search": { /* результаты от SearchAgent */ },
      "analyst": { /* результаты от AnalystAgent */ }
    }
  }
}
```

**Важно**: `context.previous_results` содержит результаты от предыдущих агентов в workflow.
...
# Передаёт AnalystAgent через context.previous_results
analyst_message = AgentMessage(
    ...
    payload={
        ...
        "context": {
            "session_id": "...",
            "previous_results": {
                "search": search_result  # ← Результаты от SearchAgent
            }
        }
    }
)
...
3. **Контекст** - `context.previous_results` передаёт данные между агентами
