...
| `research` | Поиск с ИИ             | `ResearchSource` → `ResearchController` → Orchestrator (discovery → research → structurizer → analyst → reporter) | ✅ multi-agent |
| `manual`   | Ручной ввод             | `ManualSource` — table constructor                            | ✅ реализован          |
| `stream`   | WebSocket/SSE           | `StreamSource` — stub                                         | ❌ Phase 4             |

**Кнопка «Обновить данные» на карточке SourceNode**: для типов **`document`** и **`research`** серверный `POST .../refresh` **из UI не вызывается** — пользователь видит уже сохранённый `content` узла (глобальные фильтры по-прежнему применяются к отображению). Повторное извлечение документа или повторный запуск исследования — через **настройки** соответствующего диалога. Остальные типы источников при нажатии инициируют refresh на backend. Подробнее: [API.md](./API.md) §5.1.
...
├── 🔍 Поиск с ИИ  — поиск через агентов
