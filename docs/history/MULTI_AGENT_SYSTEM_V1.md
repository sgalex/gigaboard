> **⚠️ УСТАРЕВШЕЕ** — этот документ описывает V1 архитектуру Multi-Agent системы. Актуальная версия — **V2**, см. [`MULTI_AGENT_V2_CONCEPT.md`](./MULTI_AGENT_V2_CONCEPT.md) и [`ARCHITECTURE.md`](./ARCHITECTURE.md) § 4.

# GigaBoard Multi-Agent System with Dynamic Tool Development

**Последнее обновление**: 5 февраля 2026  
**Статус**: ⛔ Устаревшее (V1). Заменено на Multi-Agent V2.  
**MVP Target**: февраль 2026

> **🆕 ВАЖНО (05.02.2026)**: Добавлен **StructurizerAgent** для извлечения структуры из текста. **AnalystAgent** теперь фокусируется только на анализе. Новый пайплайн: Search → Research → **Structure** → Analyze → Report. См. [STRUCTURIZER_AGENT_CONCEPT.md](STRUCTURIZER_AGENT_CONCEPT.md).

## 📋 Changelog

### 2026-02-05 — StructurizerAgent + AnalystAgent разделение (РЕАЛИЗОВАНО!)
- ✅ **StructurizerAgent (NEW!)**: Новый агент для извлечения структуры из неструктурированного текста
- ✅ **Разделение ответственности**: StructurizerAgent (извлечение) vs AnalystAgent (анализ)
- ✅ **Новый пайплайн**: Search → Research → **Structure** → Analyze → Report
- ✅ **StructurizerAgent выход**: `{tables[], entities[], key_value_pairs{}, extraction_confidence}`
- ✅ **AnalystAgent новый формат**: `{insights[], recommendations[], data_quality_issues[], confidence}`
- ✅ **Engine интеграция**: Task enrichment для передачи данных между агентами
- ✅ **PlannerAgent паттерны**: Обновлены все примеры с новым шагом structurizer
- ✅ **ReporterAgent**: Интеграция insights/recommendations в визуализацию
- ✅ **CriticAgent интеграция**: Добавлена валидация результатов ПОСЛЕ выполнения всех шагов
- 📚 Документация: [STRUCTURIZER_AGENT_CONCEPT.md](./STRUCTURIZER_AGENT_CONCEPT.md)

**Принцип Single Responsibility**:
- **StructurizerAgent**: Текст/HTML → Таблицы, сущности, key-value пары
- **AnalystAgent**: Структурированные данные → Insights, рекомендации, проблемы качества
- **CriticAgent**: Валидация соответствия результатов запросу пользователя
...
### 2026-01-28 — Search → Research → Analyze Pattern (УСТАРЕЛО, см. 2026-02-05)
...
- ✅ MultiAgentEngine: умная передача данных между агентами
- ✅ Протестировано: 4/5 страниц загружено успешно, 20KB данных для анализа
- ✅ Время выполнения: ~30 секунд для полного workflow (Search → Research → Structure → Analyze → Report)
...
- ✅ Документация postMessage API для ContentNode ↔ WidgetNode
...
- Описание 9 агентов и их ролей (обновлено: сейчас 12 агентов)
...
**AI Assistant Panel** работает на **Multi-Agent системе** с двумя путями вызова:

- **12 специализированных агентов** с отдельными ролями совместно решают задачи пользователя
- Агенты могут **динамически генерировать и выполнять код** для создания инструментов
- Инструменты взаимодействуют с БД, API, веб-сервисами и **публичными источниками данных**
- **Два пути выполнения**: Engine (прямые вызовы) и Orchestrator (через Message Bus)
- Все взаимодействия логируются и безопасны

**Agent roster** (12 агентов):
1. **Planner** — orchestrates workflow, delegates tasks
2. **Search** — поиск информации в интернете (DuckDuckGo)
3. **Researcher** — fetches data from URLs, APIs, web pages
4. **Structurizer** (NEW!) — извлечение структуры из текста → таблицы, сущности, key-value
5. **Analyst** — анализ структурированных данных → insights, рекомендации
6. **Transformation** — creates ContentNode transformations with Python/pandas code
7. **Resolver** — batch AI resolution для семантических задач (пол по имени, sentiment, категоризация)
8. **Critic** — валидация результатов на соответствие требованиям пользователя
9. **Form Generator** — creates dynamic input forms
10. **Executor** — runs tools in sandboxed environment
11. **Reporter** — generates WidgetNode visualizations with HTML/CSS/JS
12. **Data Discovery** — finds public datasets for users without data

---

## Architecture

### Двойной путь выполнения (Dual-Path Architecture)

В системе используется **единый путь** выполнения агентов через Orchestrator V2 (Single Path).

> **Обновлено (02.2026)**: Dual-Path (Engine + Orchestrator) упразднён. Осталась только Path B — Orchestrator + MessageBus.
> Формат передачи данных: `pipeline_context` (мутабельный dict) + `agent_results` (хронологический list).
> См. [`history/2026-02-17_CONTEXT_ARCHITECTURE_IMPLEMENTED.md`](history/2026-02-17_CONTEXT_ARCHITECTURE_IMPLEMENTED.md)

```mermaid
flowchart TB
    FE["FRONTEND: AI Assistant Panel"]
    Controllers["Satellite Controllers"]
    FE -->|WebSocket / REST| Controllers

    subgraph ORCHESTRATOR["Orchestrator V2 (Single Path)"]
        direction TB
        ORCH["MultiAgentOrchestrator\n• process_request()\n• pipeline_context (мутабельный dict)\n• agent_results (хронологический list)"]
        MB["AgentMessageBus\n(Redis Pub/Sub, DB=1)\n• request_response()\n• store_session_result()"]
        ORCH -->|"message_bus.request_response()"| MB
        MB -->|"_handle_message() → process_task()"| AGENTS_B["Agents"]
    end

    Controllers -->|"_call_orchestrator()"| ORCH

    subgraph AGENTS["9 АГЕНТОВ (BaseAgent)"]
        P["Planner"]
        STR["Structurizer"]
        AN["Analyst"]
        TR["TransformCodex"]
    ...
    subgraph DATA_SHARING["Передача данных между агентами"]
        PC["pipeline_context\n(мутабельный dict)"]
        AR["agent_results\n(хронологический list)"]
        EC["execution_context\n(DataFrame, тяжёлые данные)"]
    end
```

#### Когда используется

| Компонент           | Класс                    | Используется в            | Передача данных                                            |
| ------------------- | ------------------------ | ------------------------- | ---------------------------------------------------------- |
| **Orchestrator V2** | `MultiAgentOrchestrator` | Все Satellite Controllers | `pipeline_context` + `agent_results` + `execution_context` |

#### Единый интерфейс: `process_task()`

Все 9 агентов наследуют `BaseAgent` и реализуют единый метод:

```python
class BaseAgent(ABC):
    @abstractmethod
    async def process_task(self, task: dict, context: dict = None) -> AgentPayload:
        """Единый интерфейс для всех агентов.
        
        context содержит:
        - pipeline_context: мутабельный dict (session_id, board_id, ...)
        - agent_results: List[Dict] — хронологический список предыдущих результатов
        - execution_context: Dict — тяжёлые данные (DataFrame), не для промптов
        """
        pass
    
    # Хелперы для работы с agent_results
    def _last_result(self, context, agent_name=None) -> Optional[Dict]
    def _all_results(self, context, agent_name=None) -> List[Dict]
```

**Важно**: Агенты **не общаются друг с другом напрямую** через pub/sub каналы. 
Вся маршрутизация проходит через Orchestrator:

```
# Orchestrator V2 (единый путь):
Orchestrator.process_request()
  → PlannerAgent.process_task(task)                    # создаёт план
  → agent_results.append({"agent": "planner", "payload": plan_result})
  
  → loop: для каждого шага плана
    → agent.process_task(task, {pipeline_context, agent_results})
    → agent_results.append({"agent": step.agent, "payload": result})
  
  → ValidatorAgent.process_task(task, {pipeline_context, agent_results})
  → return final AgentPayload
```
...
**Ключевые правила**:
1. **ContentNode — первичен**: Данные всегда хранятся в ContentNode
2. **WidgetNode — производный**: Всегда создаётся от ContentNode через VISUALIZATION edge
3. **Edges описывают трансформации**: TRANSFORMATION, VISUALIZATION, COMMENT
4. **Автообновление**: Изменение ContentNode → автоматическое обновление зависимых нод
5. **Data Lineage**: Полная история трансформаций и зависимостей
6. **Sandbox execution**: Весь код выполняется в изолированной среде

### 🔄 Критический паттерн: Search → Research → Structure → Analyze

**Реализовано**: 28 января 2026, **обновлено**: 5 февраля 2026

Когда SearchAgent находит URL в интернете, выполняется 5-шаговый пайплайн:
...
2. ResearcherAgent (fetch_urls)
   ...
   → Обрабатывает до 5 URL параллельно
    ↓
3. StructurizerAgent (extract_structure)  ← 🆕 НОВЫЙ ШАГ!
   → Извлекает структуру из неструктурированного текста
   → Выход: tables[], entities[], key_value_pairs{}
   → Только извлечение, БЕЗ анализа
    ↓
4. AnalystAgent (analyze_data)
   → Анализирует структурированные данные
   → Выход: insights[], recommendations[], data_quality_issues[]
   → Только анализ, БЕЗ извлечения
    ↓
5. ReporterAgent (create_visualization)
   → Визуализация с учётом insights и recommendations
```

**Почему разделили Structure и Analyze**:
- **Single Responsibility**: Каждый агент делает одну задачу хорошо
- **Независимая оптимизация**: Промпты можно тюнить отдельно
- **Прозрачный debug**: Проблемы извлечения vs проблемы анализа разделены
- **Переиспользование**: StructurizerAgent можно использовать без Analyst

**Реализация**: PlannerAgent автоматически добавляет StructurizerAgent между ResearcherAgent и AnalystAgent

**Документация для изучения**:
- [DATA_NODE_SYSTEM.md](./DATA_NODE_SYSTEM.md) — архитектура SourceNode/ContentNode и типы контента
...
```mermaid
flowchart LR
    U[User creates SourceNode] -->|промпт| PE[PromptExtractor.extract]
    PE -->|user_message| ORC[MultiAgentOrchestrator]
    ORC -->|analyze request| PLA[PlannerAgent]
    
    PLA -->|decides workflow| CHOICE{Request Type?}
    
    CHOICE -->|"Найди..."| SEARCH[Search → Research → Structure → Analyze]
    CHOICE -->|"Создай таблицу..."| DIRECT[AnalystAgent direct]
    CHOICE -->|"Загрузи URL..."| API[Research → Structure → Analyze]
    CHOICE -->|"Анализ ContentNode"| ANALYZE[Analyst → Reporter]
    
    SEARCH -->|aggregated results| AGG[_aggregate_agent_results]
    DIRECT -->|generated data| AGG
    API -->|fetched data| AGG
    ANALYZE -->|insights| AGG
    
    AGG -->|all results| CRITIC{CriticAgent}
    CRITIC -->|✅ Valid| CONV[_convert_to_content_tables]
    CRITIC -->|❌ Invalid| REPLAN[PlannerAgent replan]
    REPLAN -->|new plan| CHOICE
...
| ---------------------- | ------------------------------------------------ | ---------------------------------------------------- |
| **Поиск информации**   | "Найди статистику кино в Москве"                 | Search → Research → **Structure** → Analyze → Report |
| **Генерация данных**   | "Создай таблицу языков: Python 1991, Rust 2015"  | AnalystAgent direct (generate_data)                  |
| **Загрузка из API**    | "Загрузи данные с https://api.example.com/users" | Research → **Structure** → Analyze                   |
| **Трансформация**      | "Преобразуй таблицу: группировка по году"        | TransformationAgent                                  |
| **Анализ ContentNode** | "Предложи бизнес-анализ для входных данных"      | AnalystAgent → ReporterAgent (без Structurizer)      |

**Критические паттерны**:
- **Search → Research → Structure → Analyze**: Полный пайплайн для веб-исследований
- **Analyze ContentNode**: Для уже структурированных данных — Structurizer не нужен
...
def _check_columns(self, code: str, input_schemas: list[dict]) -> dict:
    """Проверяет, что все используемые колонки существуют в input ContentNode"""
...
- **Transformation Agent**: Create ContentNode-to-ContentNode transformations with Python code
...
- Always create ContentNode first for any data operation
- WidgetNode requires parent ContentNode (VISUALIZATION edge)
- Transformations create new ContentNode with TRANSFORMATION edge
...
**OUTPUT REQUIREMENTS**:
Always create ContentNode with fetched data:
```json
{
...
**PRIMARY ROLE**: Generate Python code for ContentNode-to-ContentNode transformations.

**CORE CONCEPT**: 
You create data pipelines on the canvas. Each transformation is represented as:
- **Source ContentNode(s)**: Input data (can be multiple)
- **TRANSFORMATION edge**: Contains Python code (pandas operations)
- **Target ContentNode**: Output data (automatically created)
...
**AUTO-REPLAY**:
Your transformations automatically re-execute when source ContentNode(s) update:
- Parent ContentNode refreshed → Transformation re-runs → Target ContentNode updates
- Enable users to build reactive data pipelines

**DATA LINEAGE**:
Track full transformation history:
- Which ContentNodes were used as input
- What transformation code was applied
- When transformation was executed
- What was the output schema

See [DATA_NODE_SYSTEM.md](./DATA_NODE_SYSTEM.md) for SourceNode/ContentNode architecture details.
"""
```
...
**PRIMARY ROLE**: Generate WidgetNode visualizations from ContentNode using HTML/CSS/JS code rendered in iframe.
...
<body>
    ...
    <script>
        // Receive data from parent window (ContentNode)
...
**AUTO-REFRESH**:
When parent ContentNode updates:
...
**INPUT**: ContentNode with structured data (CSV, JSON, database table)
...
**BEST PRACTICES**:
1. Always provide evidence (which ContentNode, which metrics)
...
4. Filter forms (dynamic filters based on ContentNode schema)
...
5. Load selected dataset into ContentNode
...
# Researcher: "ContentNode not found"
# ↓
...
**Role**: Creates ContentNode transformations with Python code

```python
class TransformationAgent:
    """
    Generates Python code for data transformations.
    Creates TRANSFORMATION edges between ContentNodes.
    ...
    2. Analyze source ContentNode(s) schema and data
    3. Generate Python code (pandas) for transformation
    4. Create target ContentNode to hold transformed data
    ...
    - ContentNode contains data from various sources
    - TRANSFORMATION edge contains Python code (df → df)
    - Multiple ContentNodes can be inputs to a transformation
    - Transformations auto-replay when source ContentNode updates
    ...
    capabilities = {
        'analyze_content_node_schema': True,
    ...
    async def create_transformation(
        ...
        """
        Generate transformation code and create new ContentNode.
        
        Args:
            source_node_ids: List of source ContentNode IDs
            user_prompt: User description of transformation
            board_id: Target board
        
        Returns:
            {
                'target_node_id': 'content_node_456',
                'transformation_code': 'df_result = df[df["sales"] > 1000]',
                'edge_id': 'edge_789',
                'execution_status': 'success'
            }
        """
        # 1. Load source ContentNode(s)
        ...
        # 6. Create target ContentNode
        target_node = await self.create_content_node(
            board_id=board_id,
            data=result_data,
...
### 7. **Reporter Agent**
**Role**: Generates WidgetNode visualizations from ContentNode

```python
class ReporterAgent:
    """
    Generates WidgetNode (visualizations) from ContentNode.
    Creates complete HTML/CSS/JS code for any visualization type.
    Auto-refreshes visualizations when parent ContentNode updates.
    """
    
    SYSTEM_PROMPT = """
    You are the Reporter Agent. Your role:
    1. Receive ContentNode with data and schema
    ...
    5. Set up VISUALIZATION edge (ContentNode → WidgetNode) with auto-refresh
...
**Responsibilities**:
- Analyze ContentNode structure
- Generate WidgetNode with HTML/CSS/JS code
- Create VISUALIZATION edges
- Set up auto-refresh when parent ContentNode changes
- Support multiple visualizations from single ContentNode
...
### Обзор: единая система коммуникации (V2)

> **Обновлено (02.2026)**: Dual-Path (Engine + Orchestrator) упразднён.  
> Все агенты вызываются через Orchestrator V2 + MessageBus (Redis Pub/Sub).  
> Формат данных: `pipeline_context` + `agent_results` (хронологический list) + `execution_context`.

Агенты **не общаются друг с другом напрямую** — вся маршрутизация идёт через Orchestrator.

```mermaid
flowchart LR
    subgraph ORCH_PATH["Orchestrator V2 (единый путь)"]
        O[MultiAgentOrchestrator] -->|"request_response()"| MB[AgentMessageBus\nRedis Pub/Sub]
        MB -->|"_handle_message()"| A1[Agent 1]
        A1 -->|"AgentPayload"| MB
        MB -->|result| O
        O -->|"agent_results.append()"| AR["agent_results\n(хронологический list)"]
        AR -->|"context"| A2[Agent 2]
    end
```

**Механизмы передачи данных**:

| Механизм                          | Хранение                 | Доступ                                                         |
| --------------------------------- | ------------------------ | -------------------------------------------------------------- |
| `agent_results` list              | In-memory (Orchestrator) | `context["agent_results"]`, `_last_result()`, `_all_results()` |
| `pipeline_context` dict           | In-memory (Orchestrator) | `context["pipeline_context"]["key"]`                           |
| `execution_context` dict          | In-memory (Orchestrator) | `context["execution_context"]["input_data"]` (DataFrame)       |
| Redis Hash (`get_agent_result()`) | Redis DB 1               | `BaseAgent.get_agent_result(session_id, agent_name)`           |
| Redis Pub/Sub (TASK_RESULT)       | Redis channels           | `message_bus.request_response()`                               |

### Message Bus Architecture — Quick Reference

**Основа**: Redis Pub/Sub + asyncio. Используется Orchestrator V2 для всех вызовов агентов.

> ℹ️ **Orchestrator V2** — единый путь вызова всех агентов через MessageBus.
...
#### Паттерны коммуникации (Message Bus)

> **Текущая реализация**: В коде используется преимущественно паттерн **REQUEST-RESPONSE** через `message_bus.request_response()`. 
> Orchestrator отправляет задачу конкретному агенту и ждёт ответ. Broadcast и Direct пока не используются активно.

```
1. REQUEST-RESPONSE (основной паттерн)
   Orchestrator → message_bus.request_response("agent_name", task)
   ↓
   Agent._handle_message() → process_task() → TASK_RESULT
   ↓
   Orchestrator получает результат

2. BROADCAST (зарезервирован)
   User Request → gigaboard:board:{board_id}:agents:broadcast
   ↓
   Все подписанные агенты получают

3. DIRECT (Point-to-Point, зарезервирован)
   gigaboard:agents:{agent_name}:inbox
   ↓
   Только указанный агент получает
...
#### Пример полного flow (Path B — Orchestrator)

> В Path B, Orchestrator медиирует все вызовы. Агенты не отправляют сообщения друг другу напрямую.

```
1. User: "Создай график продаж"
   ↓
2. AIService.chat_stream() → Orchestrator.process_user_request()
   ↓
3. Orchestrator → message_bus.request_response("planner", task)
   ↓
4. Planner._handle_message() → process_task() → возвращает план
   ↓
5. Orchestrator получает план, итерирует по шагам
   ↓
6. Orchestrator → message_bus.request_response("researcher", step_task)
   ↓
7. Researcher._handle_message() → process_task() → данные
   ↓
8. Orchestrator → message_bus.request_response("reporter", step_task)
   ↓
9. Reporter._handle_message() → process_task() → виджет
   ↓
10. Orchestrator агрегирует результаты → Socket.IO → Frontend
```
...
> Каналы определены в `redis_config.py`. Реально используются в `message_bus.py` и `orchestrator.py`.
...
# - gigaboard:agents:transformation:inbox
...
**Routing Logic (актуальная реализация)**:
```
User Request → AIService.chat_stream()
  ↓
  Orchestrator.process_user_request()
  ↓
  Orchestrator._request_plan()  
    → message_bus.request_response("planner", task)
  ↓
  Planner._handle_message() → process_task() → план шагов
  ↓
  Orchestrator итерирует по шагам плана:
    for step in plan.steps:
      → message_bus.request_response(step.agent_name, step.task)
      → Agent._handle_message() → process_task() → результат
      → Orchestrator сохраняет результат
  ↓
  Orchestrator агрегирует → CriticAgent валидирует (standalone)
  ↓
  Результат → Socket.IO → Frontend
```
...
> Актуальная реализация: Orchestrator медиирует все вызовы через `request_response()`.
...
sequenceDiagram
    ...
    participant CR as CriticAgent (standalone)

    U->>AI: "Создай график продаж"
    AI->>O: process_user_request(message)
    
    Note over O: Step 1: Получить план
    O->>MB: request_response("planner", task)
    MB->>P: _handle_message(AgentMessage)
    P->>P: process_task(task)
    P->>MB: TASK_RESULT(plan)
    MB->>O: return result

    Note over O: Step 2: Выполнить шаг "researcher"
    O->>MB: request_response("researcher", step_task)
    MB->>R: _handle_message(AgentMessage)
    R->>R: process_task(task)
    R->>MB: TASK_RESULT(data)
    MB->>O: return result

    Note over O: Step 3: Выполнить шаг "reporter"
    O->>MB: request_response("reporter", step_task)
    MB->>Rep: _handle_message(AgentMessage)
    Rep->>Rep: process_task(task)
    Rep->>MB: TASK_RESULT(widget)
    MB->>O: return result

    Note over O: Валидация результатов
    O->>CR: process_task(aggregated_results)
    CR->>O: validation_result
    
    O->>AI: aggregated response
    AI->>U: Socket.IO stream
```
...
task_request_msg = AgentMessage(
    ...
    payload={
        ...
        "task_description": "Получить данные о продажах из ContentNode 'Sales Q1 2026'",
        "parameters": {
            "content_node_id": "node_abc123",
...
result_msg = AgentMessage(
    ...
    success=True,
    payload={
        "result_type": "content_node_created",
        "content_node_id": "node_xyz789",
        "data_preview": {
            "rows": 10000,
        ...
        "actions": [
            {
                ...
                "params": {
                    "widget_type": "chart",
                    "source_content_node_id": "node_xyz789",
                    "chart_config": {...}
                },
                "priority": 1
            },
...
2. **PLANNER AGENT** receives request
    - Task 1: Fetch sales data by region (→ Researcher → SourceNode)
    - Task 2: Calculate growth metrics (→ Transformation Agent → ContentNode)
...
3. **RESEARCHER AGENT**
    ...
    - Create **SourceNode** with raw sales data
    - SourceNode created on canvas with SQL query as source
...
5. **TRANSFORMATION AGENT**
    - Analyzes source ContentNode schema
    - Generates Python code to calculate growth:
      ...
      ```
    - Creates target **ContentNode** with growth metrics
    - Creates **TRANSFORMATION edge** with code
    - Executes transformation, populates target ContentNode

6. **ANALYST AGENT**
    - Receives transformed ContentNode
...
7. **REPORTER AGENT**
    - Receives ContentNode with growth metrics
    - Generates HTML/CSS/JS code for bar chart visualization
    - Creates **WidgetNode** with visualization code
    - Creates **VISUALIZATION edge** (ContentNode → WidgetNode) with auto-refresh
    - Creates second **WidgetNode** with recommendations text
    - Both widgets appear on canvas

8. **FRONTEND**
    - Displays agent conversation in AI Panel
    - Shows progress: SourceNode → Transformation → ContentNode → WidgetNode
    - Canvas updates: 1 SourceNode + 1 ContentNode + 2 WidgetNodes + 2 Edges appear
    - User sees full data lineage graph
    - Future: If SourceNode refreshes, transformation and visualization auto-update
...
- Создание SourceNode с fetched data
- Error handling (timeout, 404, auth failed)

**Acceptance Criteria**:
- ✅ Может выполнить SQL query → создать SourceNode
- ✅ Может сделать HTTP GET → создать SourceNode
...
- Создание target ContentNode + TRANSFORMATION edge

**Acceptance Criteria**:
- ✅ Генерирует код: `df_result = df[df['sales'] > 1000]`
- ✅ Создаёт новый ContentNode с результатом
...
**Что реализовать**:
- Анализ ContentNode (statistics, patterns, anomalies)
...
**Demo**: 
- Researcher → fetch API data → create SourceNode
- Transformation → filter data → create new ContentNode
...
**Тест-кейсы**:
1. **Full flow**: USER_REQUEST → Planner → Researcher → Structurizer → Analyst → Reporter → WidgetNode
...
5. **Complex transformation**: ContentNode → Transformation → ContentNode → Reporter → WidgetNode
6. **Discussion mode**: ContentNode → Analyst (no Structurizer) → Reporter
...
- ✅ На canvas появляется ContentNode + WidgetNode за <10s
...
**Status**: Phase 1-2 реализованы, Dual-Path Architecture (Engine + Orchestrator) активна
**Date**: 2026-02-05
