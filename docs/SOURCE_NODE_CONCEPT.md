...
| Поиск с ИИ             | 🔍      | Deep research            | Search → Research → Analyze |
...
**Специфика (актуально для JSON Source)**: автонормализация JSON в `mapping_spec` + ручная корректировка пользователем.

### Лейаут диалога

- **Левая секция**:
  - выбор JSON-файла;
  - предпросмотр исходного JSON в дереве (узлы можно разворачивать/сворачивать).
- **Правая секция**:
  - табы таблиц мэппинга;
  - операции по табам: выбор, переименование (через кнопку), удаление;
  - редактор активной таблицы (`Base path`, столбцы `path/name/type`);
  - встроенный preview активной таблицы внутри того же рамочного блока.

### Логика

1. Пользователь загружает JSON-файл.
2. Система строит `schema_snapshot` (сэмплирование массива по нескольким элементам, а не только первый).
3. На основе схемы автоматически формируется `mapping_spec` (1..N таблиц + PK/FK).
4. Пользователь корректирует таблицы/столбцы.
5. При сохранении `mapping_spec` и `schema_snapshot` сохраняются в `SourceNode.config`.
6. `refresh` переиспользует сохранённый `mapping_spec`.

### Актуальные особенности автогенерации

- Поддерживаются nested-массивы вида `...[*][*]`.
- Автоматически создаются parent-child связи (FK на ближайшего предка).
- Имена PK/FK генерируются в человекочитаемом виде с защитой от конфликтов.
- Добавление столбца выполняется по кнопке у `Base path` и автоматически прокручивает список к новому столбцу.
- Для вариативных ключей возможна канонизация алиасов полей (например, `докtype` -> `doktype`).
...
> **Исключение**: В диалоге «Поиск с ИИ» AI-ассистент остаётся слева, так как здесь нет исходных данных для превью — ассистент является основным элементом интерфейса.

**Редактирование с доски**: пункт «Настройки» на карточке SourceNode типа `research` открывает `ResearchSourceDialog` с `existingSource` и `mode="edit"`: подставляются `conversation_history`, превью результата из `content`, сохранение через `PUT` источника (кнопка «Сохранить»). Создание с витрины — режим `create`, кнопка «Создать источник».

**Превью запроса на карточке** (строка под заголовком): для отображения используются `config.initial_prompt` и при необходимости первая user-реплика из `conversation_history` (поле `query` в конфиге не используется).

**Кнопка «Обновить данные» на карточке**: для типов **`document`** и **`research`** отключена — повторное серверное извлечение из этой кнопки не запускается; на карточке показываются уже сохранённые в узле данные (фильтры доски/проекта не затрагиваются). Повторная обработка — через настройки источника. См. [API.md](./API.md) §5.1.
...
- [x] Excel Dialog
- [ ] Document Dialog
- [x] Диалог «Поиск с ИИ» (реализовано; план и детали: [AI_RESEARCH_SOURCE_IMPLEMENTATION_PLAN.md](./AI_RESEARCH_SOURCE_IMPLEMENTATION_PLAN.md))
